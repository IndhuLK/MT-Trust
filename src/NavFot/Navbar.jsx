    ...
    <>
      <nav className="relative w-full flex items-center justify-between px-20 py-2 bg-[#06457A] z-10 font-Poppins">
        ...
        <div className="hidden md:flex items-center space-x-10">
          {menuOptions.map((item, index) => (
            <Link key={index} to={item.path} className="relative text-white hover:text-yellow-400 font-semibold group">
              {item.name}
        ...
        <div className="md:hidden flex items-center ml-auto">
          {openMenu ? (
            <HiX size={30} className="text-white cursor-pointer" onClick={() => setOpenMenu(false)} />
          ) : (
            <HiOutlineBars3 size={30} className="text-white cursor-pointer" onClick={() => setOpenMenu(true)} />
          )}
        </div>
      </nav>
      ...
      {openMenu && (
        <div className="md:hidden fixed inset-0 bg-[#06457A] z-50 flex flex-col justify-center items-center px-4">
          <button onClick={() => setOpenMenu(false)} className="absolute top-4 right-4 text-white text-2xl">
            <HiX />
          </button>

          <div className="absolute top-4 text-yellow-400 font-bold text-3xl">MTSSDS</div>

          <div className="flex flex-col space-y-9 text-white text-lg text-center">
            {menuOptions.map((item, index) => (
              <Link key={index} to={item.path} className="hover:text-yellow-400" onClick={() => setOpenMenu(false)}>
                {item.name}
              </Link>
            ))}
          </div>
